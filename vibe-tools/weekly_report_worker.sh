#!/usr/bin/env bash
# weekly_report_worker.sh — 주간보고 송부용 초안을 무인 생성하는 금요일 워커
#
# 금요일 오후 15:50 launchd 에 의해 실행됨 (16:00 메신저 송부 10분 전, caffeinate -i -s 래핑).
#
# 동작:
#   PARA 볼트를 cwd 로 두고 헤드리스 claude 에 `/weekly-report` 을 던져,
#   그 산출물(메신저 송부용 텍스트 + 검토 체크리스트)을 파일로 저장한다.
#
# 🔴 작성 규칙은 본 스크립트에 없다 — 정본은 PARA 로컬 스킬이다:
#   $PARA_VAULT/.claude/skills/weekly-report/SKILL.md
#   (형식 정본은 그 스킬이 참조하는 templates/주간보고_메신저_템플릿.md)
#   워커는 그 스킬을 호출하는 실행기일 뿐이므로, 보고 형식·진행률 규칙을 여기에 옮겨 적지 말 것.
#   스킬이 project-local 이라 cwd 를 PARA 볼트로 두지 않으면 스킬이 잡히지 않는다.
#
# 🔴 무인 완결이 아니다 — notion_diary_worker 와 다른 점:
#   산출물은 "사람이 검토해 메신저로 송부하는 텍스트"다. 송부는 사람이 한다.
#   따라서 이 워커는 (1) 파일 저장 (2) 로컬 알림 만 수행하고,
#   슬랙·메일·메신저 등 외부 발신은 절대 하지 않는다.
#
# 환경 변수:
...
#
# 산출물 / 로그:
#   ~/Library/Logs/weekly-report/YYYY-MM-DD.md    주간보고 초안 (송부용)
#   ~/Library/Logs/weekly-report/YYYY-MM-DD.log   실행 로그
#
# 안전 가드:
#   - 스킬 1~3단계(GROUND~VERIFY)는 PARA 노트 읽기 위주. 4단계 ADAPT(노트 갱신)는
#     "사용자 송부 완료 보고 후" 단계라 무인 실행에서는 자연히 수행되지 않는다.
#   - 토·일 결정론 skip (plist 는 금요일만 돌지만 수동 실행 대비).
#   - launchd 에서 caffeinate 로 래핑하므로 이 스크립트는 직접 caffeinate 호출 안 함.
#
# 사용법:
#     DRY_RUN=1 ./weekly_report_worker.sh
#     PARA_VAULT=/custom/para ./weekly_report_worker.sh
#
# 수동 부트스트랩 (setup.sh 는 plist 를 배포하지 않음 — SoC):
#   cp vibe-tools/com.swkim0128.weekly-report.plist ~/Library/LaunchAgents/
#   launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.swkim0128.weekly-report.plist
#   (해제: launchctl bootout gui/$(id -u)/com.swkim0128.weekly-report)
...
DRY_RUN="${DRY_RUN:-0}"
SKILL_FILE="${PARA_DIR}/.claude/skills/weekly-report/SKILL.md"
...
announce_output() {
  log_info "┌───────────────────────────────────────────────────────────"
  log_info "│ 주간보고 초안 산출물: ${OUT_FILE}"
  log_info "└───────────────────────────────────────────────────────────"
}

# ── 시작 로그 ────────────────────────────────────────────────────────────────
log_info "===== weekly_report_worker 시작 ====="
log_info "실행 시각: ${RUN_DATE}"
log_info "PARA 볼트: ${PARA_DIR}"
log_info "대상 날짜: ${TODAY}"
log_info "DRY_RUN: ${DRY_RUN}"

# ── 주말 결정론 skip (주간보고는 평일 송부) ─────────────────────────────────
if [[ "${DOW_NUM}" == "6" || "${DOW_NUM}" == "7" ]]; then
  log_info "오늘(${TODAY})은 주말 — 주간보고 송부일이 아니라 skip 합니다."
  log_info "===== weekly_report_worker 완료 (주말 skip) ====="
  exit 0
fi
...
if [[ ! -f "${SKILL_FILE}" ]]; then
  log_error "weekly-report 스킬 정본이 없습니다: ${SKILL_FILE}"
  exit 1
fi
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  log_info "[DRY_RUN] 실행 예정: (cd ${PARA_DIR}) ${CLAUDE_BIN:-<claude 미발견>} --print /weekly-report"
  log_info "[DRY_RUN] 산출물 예정 경로: ${OUT_FILE}"
  log_info "===== weekly_report_worker 완료 (DRY_RUN) ====="
  exit 0
fi
...
if [[ ${CLAUDE_EXIT} -ne 0 ]]; then
  ...
  notify "주간보고 초안 실패" "exit ${CLAUDE_EXIT} — 로그: ${LOG_FILE}"
  exit ${CLAUDE_EXIT}
fi

if [[ ! -s "${OUT_FILE}" ]]; then
  log_error "claude 호출은 성공했으나 산출물이 비어 있습니다: ${OUT_FILE}"
  notify "주간보고 초안 비어 있음" "스킬 인식 실패 의심 — 로그: ${LOG_FILE}"
  exit 1
fi

OUT_LINES="$(wc -l < "${OUT_FILE}" | tr -d ' ')"
log_info "주간보고 초안 생성 완료 (${OUT_LINES} 줄)."
announce_output
notify "주간보고 초안 준비됨 (16:00 송부)" "${OUT_FILE}"
log_info "===== weekly_report_worker 완료 ====="
exit 0
