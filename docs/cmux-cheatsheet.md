...
| **멀티 레포 이슈 워크스페이스** (1개 이슈가 2개+ 레포 동시 수정, 예: DWDEV-2959 = BillingMPAdmin 메인 + PHPLib·MAS 서브) | `cmux-issue.sh` — 이슈명 워크스페이스+tmux 세션(win1 좌측 main=para claude 추적·오케스트레이션, 우측 스택=레포별 claude 주 작업 레포 최상단, win2 view=vim 육안 열람). **단계적 플로우**: ① `cmux-issue.sh <ISSUE>`(proj 생략 = para 단독 생성) → ② para pane 에서 작업 정리·확인 → ③ 확인 후 `cmux-issue.sh --add <ISSUE> <proj…>`(프로젝트 pane 신규 스폰) 또는 para pane 에서 `vibe delegate <proj>`. 한 번에 필요하면 `cmux-issue.sh <ISSUE> <proj…>` 선생성도 가능. 실행 중 pane 무중단 이전 → `cmux-issue.sh --join <ISSUE> <pane_id …>`(join-pane 프로세스째). proj 는 등록명 또는 raw 경로 | 개별 `vibe delegate`(여러 cwd 동시 불가)·raw `tmux new-session` 금지 / 단일 레포는 `프로젝트명_이슈번호` 세션 또는 `vibe delegate` 유지 / cmux 미설치·제외목록 → tmux-only 폴백 |
