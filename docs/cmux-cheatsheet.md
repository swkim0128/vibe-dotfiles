...
| **멀티 레포 이슈 워크스페이스** = 복합·신규 생성 유형 (1개 이슈가 2개+ 레포 동시 수정, 예: DWDEV-2959 = BillingMPAdmin 메인 + PHPLib·MAS 서브) | **판별 기준**: 하나의 이슈가 여러 레포를 동시에 수정하는 복합 작업(단일 프로젝트 위임으로 여러 cwd 를 동시에 붙들 수 없음) → **신규 생성 유형**. `cmux-issue.sh` — 이슈명 워크스페이스+para 세션 **신규 생성**(win1 좌측 main=para claude 추적·오케스트레이션, 우측 스택=레포별 claude 주 작업 레포 최상단, win2 view=vim 육안 열람). **단계적 플로우**: ① `cmux-issue.sh <ISSUE>`(proj 생략 = para 단독 생성) → ② para pane 에서 작업 정리·확인 → ③ 확인 후 `cmux-issue.sh --add <ISSUE> <proj…>`(프로젝트 pane 신규 스폰) 또는 para pane 에서 `vibe delegate <proj>`. 한 번에 필요하면 `cmux-issue.sh <ISSUE> <proj…>` 선생성도 가능. 실행 중 pane 무중단 이전 → `cmux-issue.sh --join <ISSUE> <pane_id …>`(join-pane 프로세스째). proj 는 등록명 또는 raw 경로 | 개별 `vibe delegate`(여러 cwd 동시 불가)·raw `tmux new-session` 금지 / 단일 레포·간단 작업은 아래 「타 프로젝트 위임(기본)」 유지 / cmux 미설치·제외목록 → tmux-only 폴백 |
| 타 프로젝트 위임(기본) = 메인 오퍼레이션 | **판별 기준**: para 허브 세션·워크스페이스가 메인 오퍼레이션 — 간단한 작업·단일 프로젝트로 처리 가능하면 허브 **현재 창**에 `vibe delegate <proj>` 신규 pane 분할(현행). | 세션·워크스페이스 신규 생성 금지 / **여러 레포 동시 수정(복합)은 이 경로 대신 위 「멀티 레포 이슈 워크스페이스」로 승격** |
...
- `bash ~/.config/vibe-tools/cmux-issue.sh <ISSUE> [proj …]`  # ① 멀티 레포 이슈 (proj 생략=para 단독, para 좌·추적)
- `bash ~/.config/vibe-tools/cmux-issue.sh --add <ISSUE> <proj …>`  # ③ 확인 후 프로젝트 pane 신규 스폰(우측 스택)
- `bash ~/.config/vibe-tools/cmux-issue.sh --join <ISSUE> <pane_id …>`  # 실행 중 pane 을 이슈 우측 스택에 무중단 이전(join-pane)
