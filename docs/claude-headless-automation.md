...
- 현재 `~/.claude/settings.json`은 `vibe-claude-plugin/claude-config/settings.work.json` 심볼릭 링크 (활성 hook은 `~/.claude/hooks/`)
...
### 단계 1: 프로젝트 권한 화이트리스트
위 `.claude/settings.json` 적용 (또는 현재 심볼릭 링크된 `vibe-claude-plugin/claude-config/settings.work.json`에 반영)
...
tmux send-keys -t worker:0 \
  "bash \"\${DOTFILES_ROOT}/scripts/claude-headless.sh\" '코드 리팩토링 후 shellcheck'" Enter
```
...
- [ ] 위 스키마를 `.claude/settings.json`에 작성 (또는 `vibe-claude-plugin/claude-config/settings.work.json`에 반영)
