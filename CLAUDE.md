...
`vibe-config@personal` 플러그인은 GitHub URL을 마켓플레이스로 등록하여 설치하므로 로컬 클론이 필요하지 않습니다. `~/.claude/plugins/cache/personal/vibe-config/` 가 Claude Code에 의해 관리됩니다.
...
### `vibe-config@personal` 플러그인
개인 Claude Code 플러그인. [swkim0128/vibe-claude-plugin](https://github.com/swkim0128/vibe-claude-plugin) 레포에서 관리하며, `setup.sh` 가 GitHub URL을 `personal` 마켓플레이스로 등록하고 `claude plugin install vibe-config@personal` 로 설치합니다 (로컬 클론 불필요).

캐시 경로: `~/.claude/plugins/cache/personal/vibe-config/`
